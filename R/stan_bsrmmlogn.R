#' Bayesian Structural Risk Measurement Model - Log Normal likelihood
#'
#' @description Fit Bayesian Structural Risk Measurement Model using stan.
#' This function is a wrapper function around *bstrmmlogn.stan*.
#'
...
#'
#' @export
stan_bsrmmlogn <- function(losses, sbu, time, bu = NULL, n_t = NULL,
                           ...
                           ), H = 1, pars = c("lambdas", "mu", "sigma", "Y_hat"),
                           ...
) {

  if(any(!c("lambdas", "mu", "sigma") %in% pars)) {
    stop("pars have to consists of all of lambdas, mu, sigma to construct posterior distribution")
  }
  ...
  out <- rstan::sampling(stanmodels$bsrmmlogn, data = standata, pars = pars, ...)
  return(out)

...
transformed parameters {
  ...
  for(j in 1:J) {
    ...
    pp[j] = 1 - student_t_cdf(logH, nu[j], mu[j], sigma[j]);
  }
...
model {
  ...
  // hyper priors
  // degree of freedom has a enterprise prior
  nu ~ gamma(a_nu, b_nu);
  ...
  // t likelihood
  log_lik = student_t_lpdf(logY | nu_aux, mu_aux, sigma_aux) - log(pp_aux);
  ...
  for(j in 1:J) {
	...
	if(lambda_hat == 0) {
	  ...
	  for(k in 1:lambda_hat) {
	    Y_hat[j] += exp(student_t_rng(nu[j], mu[j], sigma[j]));
	  }
	}
  }
}
